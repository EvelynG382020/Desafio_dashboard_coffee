class DashboardController < ApplicationController
  def index
    @sales_by_month = SaleCoffe.group_by_month(:date_time, last: 12).order(date_time: :asc).sum(:price)
    @sales_by_count = SaleCoffe.group_by_month(:date_time, last: 12).order(date_time: :asc).count
    @sales_by_average = SaleCoffe.group_by_month(:date_time, last: 12).order(date_time: :asc).average(:price)
